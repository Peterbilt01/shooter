...
sky_imag=pygame.image.load('img/Background/sky_cloud.png').convert_alpha()
...
def draw_bg():
	width=sky_imag.get_width()
	for x in range(6):
		screen.blit(sky_imag,((x*width)-bg_scroll*0.5,0))
	...
	def move(self, moving_left, moving_right):
		...
		for tile in world.obstacle_list:
			...
			if tile[1].colliderect(self.rect.x + dx, self.rect.y, self.rect.width, self.rect.height):
				dx = 0
				#if the ai hit the wall then makes it turn around
		...
		for tile in world.obstacle_list:
			...
			if tile[1].colliderect(self.rect.x, self.rect.y + dy, self.rect.width, self.rect.height):				
				...
				#check if above the ground i.e. falling
					...
					dy = tile[1].top - self.rect.bottom
					self.jump = True
					while self.speed > 0 and self.jump == True:
...
class ScreenFade():
	...
	def fade(self):
		...
		if self.fade_counter >= SCREEN_WIDTH-100:
			fade_complete=True
